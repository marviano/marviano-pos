    ...
    try {
      ...
      if (printResult?.success) {
        ...
        if (originalPrinterType === 'receiptPrinter') {
          ...
          const logResult = await logPrinter2?.(
            transaction.id,
            originalCounter,
            'manual',
            undefined,
          ...
          // Queue transaction for System POS sync after successful audit log
          if (logResult?.success) {
            try {
              await electronAPI.queueTransactionForSystemPos?.(transaction.id);
            } catch (queueError) {
              console.error('❌ Error queueing transaction for System POS:', queueError);
              // Don't fail the reprint if System POS queue fails
            }
          }
        }
                ...
                {isLoading ? (
                  ...
                  <>
                    ...
                    {transaction?.transaction_type && (
                      <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${transaction.transaction_type === 'drinks'
