...
contextBridge.exposeInMainWorld('electronAPI', {
  ...
  getRestaurantSections: (roomId: number) => ipcRenderer.invoke('get-restaurant-sections', roomId),
  createRestaurantSection: (payload: { room_id: number; name: string; color?: string }) => ipcRenderer.invoke('create-restaurant-section', payload),
  updateRestaurantSection: (payload: { id: number; name?: string; color?: string }) => ipcRenderer.invoke('update-restaurant-section', payload),
  deleteRestaurantSection: (sectionId: number) => ipcRenderer.invoke('delete-restaurant-section', sectionId),
  updateRestaurantTable: (payload: { id: number; table_number?: string; capacity?: number; shape?: string; section_id?: number | null }) => ipcRenderer.invoke('update-restaurant-table', payload),
  localDbUpsertRestaurantRooms: (rows: UnknownRecord[]) => ipcRenderer.invoke('localdb-upsert-restaurant-rooms', rows),
  localDbUpsertRestaurantSections: (rows: UnknownRecord[]) => ipcRenderer.invoke('localdb-upsert-restaurant-sections', rows),
  ...
  /** GET http(s) from main process (avoids CORS when calling FlexCode endpoints from Electron). */
