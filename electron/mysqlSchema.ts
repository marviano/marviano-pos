...
export async function initializeMySQLSchema(): Promise<void> {
  ...
  const tables = [
    ...
    `CREATE TABLE IF NOT EXISTS product_customization_options (
      ...
      rental_duration_value DECIMAL(10,2) NULL,
      rental_duration_unit ENUM('hour','day','month') NULL,
      display_order INT DEFAULT '0',
    ...
    // Absensi removed (fingerprint attendance)
  ...
  try {
    ...
    try {
      await pool.execute(
        `ALTER TABLE receipt_templates ADD COLUMN show_notes TINYINT(1) DEFAULT 0 COMMENT '1 = show item note/customization on print, 0 = hide'`,
        []
      );
    ...
    try {
      await pool.execute(
        `ALTER TABLE receipt_templates MODIFY COLUMN template_type ENUM('receipt', 'bill', 'refund', 'checker') NOT NULL COMMENT 'Type of template: receipt (paid transaction), bill (unpaid order), refund, or checker (kitchen label)'`
      );
    ...
    try {
      await pool.execute(
        `ALTER TABLE transactions ADD COLUMN paid_at DATETIME DEFAULT NULL COMMENT 'When the transaction was paid (status completed/paid)' AFTER updated_at`
      );
    ...
    try {
      await pool.execute(
        `ALTER TABLE transactions ADD COLUMN customer_unit INT DEFAULT NULL COMMENT 'Number of customers / pax' AFTER customer_name`
      );
    ...
    try {
      await pool.execute(
        `ALTER TABLE products ADD COLUMN is_package TINYINT(1) NOT NULL DEFAULT 0 COMMENT '1 = package product (combo/meal deal), 0 = regular or bundle' AFTER is_bundle`
      );
    ...
    try {
      await pool.execute(
        `ALTER TABLE transaction_items ADD COLUMN package_selections_json JSON DEFAULT NULL COMMENT 'Package chosen/default items for package products' AFTER bundle_selections_json`
      );
