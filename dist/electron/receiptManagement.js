...
class ReceiptManagementService {
    ...
    async getReceiptTemplate(templateType, businessId) {
        try {
            ...
            if (businessId) {
                const businessTemplate = await (0, mysqlDb_1.executeQueryOne)(`SELECT template_code FROM receipt_templates 
           ...
           ORDER BY version DESC LIMIT 1`, [templateType, businessId]);
            ...
            const globalTemplate = await (0, mysqlDb_1.executeQueryOne)(`SELECT template_code FROM receipt_templates 
    ...
    async getReceiptTemplates(templateType, businessId) {
        try {
            const templates = await (0, mysqlDb_1.executeQuery)(`SELECT id, template_name, is_default FROM receipt_templates 
         WHERE template_type = ? AND (business_id = ? OR business_id IS NULL) AND is_active = 1 
         ORDER BY is_default DESC, template_name ASC`, [templateType, businessId || null]);
    ...
    async setDefaultTemplate(templateType, templateName, businessId) {
        try {
            ...
            await (0, mysqlDb_1.executeUpdate)(`UPDATE receipt_templates 
         SET is_default = 0 
         WHERE template_type = ? AND (business_id = ? OR business_id IS NULL)`, [templateType, businessId || null]);
            // Then set the selected template as default
            const result = await (0, mysqlDb_1.executeUpdate)(`UPDATE receipt_templates 
         SET is_default = 1 
         WHERE template_type = ? AND template_name = ? AND (business_id = ? OR business_id IS NULL) AND is_active = 1`, [templateType, templateName, businessId || null]);
            if (result > 0) {
     ...
     */
    async saveReceiptTemplate(templateType, templateCode, templateName, businessId) {
        try {
         ...
         ORDER BY version DESC LIMIT 1`, businessId ? [templateType, templateName || 'Default', businessId] : [templateType, templateName || 'Default']);
            ...
            await (0, mysqlDb_1.executeUpsert)(`INSERT INTO receipt_templates (template_type, template_name, business_id, template_code, is_active, version, updated_at)
         VALUES (?, ?, ?, ?, 1, ?, NOW())
         ON DUPLICATE KEY UPDATE 
           template_code = VALUES(template_code),
           version = VALUES(version),
           is_active = 1,
           updated_at = NOW()`, [templateType, templateName || 'Default', businessId || null, templateCode, newVersion]);
    ...
    async saveReceiptSettings(settings, businessId) {
        try {
            await (0, mysqlDb_1.executeUpsert)(`INSERT INTO receipt_settings (
